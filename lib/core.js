...
function parsePage(source, components, pushedComponents) {
  ...
  includes.forEach((componentPath) => {
    ...
    parsePage(fs.readFileSync(path.join(srcPath, parsed.dir, parsed.name, parsed.base), 'utf8'), components, pushedComponents);
  });
...
function main(source, _extractor, _ext, _srcPath, _builtJSPath, _builtCSSPath, _builtTemplatePath, _selfPath) {
  ...
  selfPath = _selfPath;
  let components = [];
  parsePage(source, components, new Map());
