...
function buildTemplate(components) {
  components.forEach((component) => {
    ...
    const templatePath = path.join(srcPath, component.dir, component.name, `${component.name}.tpl`);
    const template = fs.readFileSync(templatePath, 'utf8');
    fs.writeFile(path.join(builtTemplatePath, component.dir, `${component.name}.tpl`), template, (err) => {
      if (err) throw err;
...
function main(source, _extractor, _srcPath, _builtJSPath, _builtCSSPath, _builtTemplatePath, _selfPath) {
