...
function parsePage(source, components) {
  const includes = extractor(source);
  if (!includes) return null;
  includes.forEach((componentPath) => {
    const parsed = path.parse(componentPath);
    components.push(parsed)
    parsePage(fs.readFileSync(path.join(srcPath, parsed.dir, parsed.name, parsed.base), 'utf8'), components);
  });
